...
public class NGramReader {
    ...
    File unigramFile = new File("C:\\Users\\mhjang\\Desktop\\ngram_index\\1gms0");
    File bigramFile = new File("C:\\Users\\mhjang\\Desktop\\ngram_index\\2gms");
    File trigramFile = new File("C:\\Users\\mhjang\\Desktop\\ngram_index\\3gms_1");
    ...
    public double termCollectionProbability(String term) {
        ...
        if(tokens.length == 1) {
            data = unigramReader.get(ByteUtil.fromString(term));
            total = this.unigramTotalCount;
        }
        else if(tokens.length == 2) {
            data = bigramReader.get(ByteUtil.fromString(term));
            total = this.bigramTotalCount;
        }
        else if(tokens.length == 3) {
            data = trigramReader.get(ByteUtil.fromString(term));
            total = this.trigramTotalCount;
        }
        if(data != null) {
            String count = ByteUtil.toString(data);
    ...
    private void setTotalCount() {
      ...
      */
        ...
        for(byte[] key: bigramReader.keySet()) {
            ...
            bigramTotalCount = bigramTotalCount.add(new BigInteger(ByteUtil.toString(data)));
        }
        System.out.print("bigram: " + bigramTotalCount);

        for(byte[] key: trigramReader.keySet()) {
            byte[] data = trigramReader.get(key);
            trigramTotalCount = trigramTotalCount.add(new BigInteger(ByteUtil.toString(data)));
        }
